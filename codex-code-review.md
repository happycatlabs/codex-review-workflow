...
the one-shot mutation. GitHub rejects `resolveReviewThread` for an App token
limited to `contents: read` even when `viewerCanResolve` is true. The resolver
token therefore requests `contents: write` with `pull-requests: write`, and is
minted only when a resolve decision exists; the independent publisher token
remains limited to `contents: read`. Keeps cause no mutation.
